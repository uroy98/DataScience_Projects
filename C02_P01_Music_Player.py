...
class LinkedList:
    ...
    def delete_song(self, song_name):
        ...
        # Search for the song to be deleted, keep track of the
        ...
        # if song was not present in linked list
        if(temp == None):
            print('Song does not exist to be deleted')
            return
    ...
    def sort_list(self):
    	...
    	# Once the list is updated return result to the function from where ot was called. 
            ...
            newlist.append(self.head_node)
            while current.next:
                current = current.next
                newlist.append(current)
            #newlist = sorted(newlist, key = lambda node: node.song.song_name, reverse = False )
            newlist.sort(key = lambda node: node.song.song_name, reverse = False )
            
            newll = LinkedList()
            for node in newlist:
                newll.insert_at_start(node)
                
            #Reversing the linked list which is sorted in descending order so that the final list is sorted in
            #ascending order
            
            prev = None
            current = newll.head_node
    ...
    def shuffle_song(self):
        ...
        if self.head_node is None:
            return
        if self.head_node and not self.head_node.next:
            print ("Randomly selected song is %s" %(self.head_node.song.song_name))
        ...
        result = self.head_node.song.song_name
